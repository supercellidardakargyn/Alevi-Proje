  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        CircleAvatar(
          radius: size / 2,
          backgroundColor: AppInk.divider,
                ...
                if (profile.referred || profile.sharedEvents > 0 || profile.sharedInterests.isNotEmpty)
                  Wrap(
                    ...
                    children: [
                      if (profile.referred)
                        const Chip(
                          avatar: Icon(Icons.verified_user, size: 16, color: AppColors.burgundy),
                          label: Text('Topluluk onaylı'),
                        ),
                      if (profile.sharedEvents > 0)
                        Chip(
                          avatar: const Icon(Icons.event, size: 16, color: AppColors.burgundy),
                          label: Text('${profile.sharedEvents} ortak etkinlik'),
                        ),
                      for (final tag in profile.sharedInterests)
                        Chip(
                          avatar: const Icon(Icons.favorite, size: 16, color: AppColors.burgundy),
                          label: Text(tag),
                        ),
                    ],
                  ),
