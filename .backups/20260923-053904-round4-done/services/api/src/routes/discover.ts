...
export function discoverRoutes(prisma: PrismaClient): Router {
  ...
  router.get('/', validate(discoverQuerySchema, 'query'), asyncHandler(async (req, res) => {
    ...
    const query = req.query as unknown as { cursor?: string; limit: number; q?: string; city?: string; latitude?: number; longitude?: number; maxDistanceKm?: number };
    ...
    const limited = items.slice(0, query.limit + 1);
    const hasMore = limited.length > query.limit;
    const page = hasMore ? limited.slice(0, query.limit) : limited;
    res.json({ data: { items: page.map(toPublicProfile), nextCursor: !useGeo && hasMore ? page.at(-1)?.id ?? null : null } });
  }));
  return router;
}
